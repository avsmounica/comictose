 ...
 */
public class CalvinAndHobbesScraper {
    ...
    public static void startPoint() throws Exception {
        CodeSource codeSource = CalvinAndHobbesScraper.class.getProtectionDomain().getCodeSource();
        File jarFile = new File(codeSource.getLocation().toURI().getPath());
        String jarDir = jarFile.getParentFile().getPath();
        String folderName = "CalvinAndHobbes";
        File file = new File(jarDir + "\\" + folderName);
        file.mkdir();
        filesPath = jarDir + "\\" + folderName; //Change filepath here
        org.joda.time.LocalDate startDate = new org.joda.time.LocalDate(1985, 11, 18);
        ...
        for (int i = 0; i < NUM_OF_THREADS; i++) {
            if (i == 0) {
                ...
                threadList[i] = new CalvinAndHobbesThread(i, filesPath, start, end);
