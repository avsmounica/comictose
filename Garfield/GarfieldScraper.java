...
public class GarfieldScraper {
    ...
    public static void startPoint() throws Exception {
        CodeSource codeSource = GarfieldScraper.class.getProtectionDomain().getCodeSource();
        File jarFile = new File(codeSource.getLocation().toURI().getPath());
        String jarDir = jarFile.getParentFile().getPath();
        String folderName = "Garfield";
        File file = new File(jarDir + "\\" + folderName);
        file.mkdir();
        filesPath = jarDir + "\\" + folderName; //Change filepath here
        org.joda.time.LocalDate startDate = new org.joda.time.LocalDate(1978, 06, 19);
